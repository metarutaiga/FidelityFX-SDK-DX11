...
:hlsl
setlocal
set file=nv_fsr3upscaler%1pass%2permutations_%3_%4_%5_%6_%7_%8.hlsl
...
echo.>>%file%
echo #include "ffx_wave_nvidia.h">>%file%
echo #include "../../hlsl/fsr3upscaler/ffx_fsr3upscaler%1pass.hlsl">>%file%
goto :eof

:header
setlocal
set file=nv_fsr3upscaler%1pass%2permutations.h
if exist %file% goto :eof
echo %file%
  ...
  for %%d in (0,1) do (
    for %%e in (0,1) do (
      for %%f in (0,1) do (
        for %%g in (0,1) do (
          for %%h in (0,1) do (
            echo #include "nv_fsr3upscaler%1pass%2permutations_%%c_%%d_%%e_%%f_%%g_%%h.h">>%file%
          )
        )
      )
    )
...
echo.>>%file%
echo static const struct { const uint8_t* data; uint32_t size; } g_nv_fsr3upscaler%1pass%2permutations[2][2][2][2][2][2] = {>>%file%
for %%c in (0,1) do (
  echo     {>>%file%
  for %%d in (0,1) do (
    echo         {>>%file%
    for %%e in (0,1) do (
      ...
      for %%f in (0,1) do (
        ...
        for %%g in (0,1) do (
          ...
          for %%h in (0,1) do (
            echo                         {>>%file%
            echo                             g_nv_fsr3upscaler%1pass%2permutations_%%c_%%d_%%e_%%f_%%g_%%h, sizeof^(g_nv_fsr3upscaler%1pass%2permutations_%%c_%%d_%%e_%%f_%%g_%%h^)>>%file%
            echo                         },>>%file%
          )
          echo                     },>>%file%
        )
