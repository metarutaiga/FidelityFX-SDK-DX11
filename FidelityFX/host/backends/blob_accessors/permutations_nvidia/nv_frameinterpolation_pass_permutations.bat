...
:hlsl
setlocal
set file=nv_frameinterpolation%1pass%2permutations_%3.hlsl
...
echo.>>%file%
for %%c in (0,1) do (
  echo #include "nv_frameinterpolation%1pass%2permutations_%%c.h">>%file%
)
echo.>>%file%
echo static const struct { const uint8_t* data; uint32_t size; } g_nv_frameinterpolation%1pass%2permutations[2] = {>>%file%
for %%c in (0,1) do (
  echo     {>>%file%
  echo         g_nv_frameinterpolation%1pass%2permutations_%%c, sizeof^(g_nv_frameinterpolation%1pass%2permutations_%%c^)>>%file%
  echo     },>>%file%
)
echo };>>%file%
goto :eof
