...
:hlsl
setlocal
set file=nv_frameinterpolation%1pass%2permutations_%3_%4_%5.hlsl
...
:header
setlocal
set file=nv_frameinterpolation%1pass%2permutations.h
if exist %file% goto :eof
echo %file%
...
for %%c in (0,1) do (
  for %%d in (0,1) do (
    for %%e in (0,1) do (
      echo #include "nv_frameinterpolation%1pass%2permutations_%%c_%%d_%%e.h">>%file%
    )
  )
)
echo.>>%file%
echo static const struct { const uint8_t* data; uint32_t size; } g_nv_frameinterpolation%1pass%2permutations[2][2][2] = {>>%file%
for %%c in (0,1) do (
  echo     {>>%file%
  for %%d in (0,1) do (
    echo         {>>%file%
    for %%e in (0,1) do (
      echo             {>>%file%
      echo                 g_nv_frameinterpolation%1pass%2permutations_%%c_%%d_%%e, sizeof^(g_nv_frameinterpolation%1pass%2permutations_%%c_%%d_%%e^)>>%file%
      echo             },>>%file%
    )
    echo         },>>%file%
  )
