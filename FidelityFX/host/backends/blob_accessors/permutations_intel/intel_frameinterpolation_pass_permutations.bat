...
:hlsl
setlocal
set file=intel_frameinterpolation%1pass%2permutations_%3.hlsl
...
echo.>>%file%
for %%c in (0,1) do (
  echo #include "intel_frameinterpolation%1pass%2permutations_%%c.h">>%file%
)
echo.>>%file%
echo static const struct { const uint8_t* data; uint32_t size; } g_intel_frameinterpolation%1pass%2permutations[2] = {>>%file%
for %%c in (0,1) do (
  echo     {>>%file%
  echo         g_intel_frameinterpolation%1pass%2permutations_%%c, sizeof^(g_intel_frameinterpolation%1pass%2permutations_%%c^)>>%file%
  echo     },>>%file%
)
echo };>>%file%
goto :eof
