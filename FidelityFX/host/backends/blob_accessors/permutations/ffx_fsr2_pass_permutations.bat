...
for %%a in (_accumulate_,_autogen_reactive_,_compute_luminance_pyramid_,_depth_clip_,_lock_,_rcas_,_reconstruct_previous_depth_,_tcr_autogen_) do (
...
:hlsl
setlocal
set file=ffx_fsr2%1pass%2permutations_%3_%4_%5_%6_%7_%8.hlsl
...
echo.>>%file%
echo #include "../../hlsl/fsr2/ffx_fsr2%1pass.hlsl">>%file%
goto :eof

:header
setlocal
set file=ffx_fsr2%1pass%2permutations.h
if exist %file% goto :eof
echo %file%
  ...
  for %%d in (0,1) do (
    for %%e in (0,1) do (
      for %%f in (0,1) do (
        for %%g in (0,1) do (
          for %%h in (0,1) do (
            echo #include "ffx_fsr2%1pass%2permutations_%%c_%%d_%%e_%%f_%%g_%%h.h">>%file%
          )
        )
      )
    )
...
echo.>>%file%
echo static const struct { const uint8_t* data; uint32_t size; } g_ffx_fsr2%1pass%2permutations[2][2][2][2][2][2] = {>>%file%
for %%c in (0,1) do (
  echo     {>>%file%
  for %%d in (0,1) do (
    echo         {>>%file%
    for %%e in (0,1) do (
      ...
      for %%f in (0,1) do (
        ...
        for %%g in (0,1) do (
          ...
          for %%h in (0,1) do (
            echo                         {>>%file%
            echo                             g_ffx_fsr2%1pass%2permutations_%%c_%%d_%%e_%%f_%%g_%%h, sizeof^(g_ffx_fsr2%1pass%2permutations_%%c_%%d_%%e_%%f_%%g_%%h^)>>%file%
            echo                         },>>%file%
          )
          echo                     },>>%file%
        )
