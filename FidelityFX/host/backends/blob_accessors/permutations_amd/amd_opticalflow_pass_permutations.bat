...
:hlsl
setlocal
set file=amd_opticalflow%1%2%3permutations_%4.hlsl
...
:header
setlocal
set file=amd_opticalflow%1%2%3permutations.h
if exist %file% goto :eof
echo %file%
...
echo.>>%file%
for %%c in (0,1) do (
  echo #include "amd_opticalflow%1%2%3permutations_%%c.h">>%file%
)
echo.>>%file%
echo static const struct { const uint8_t* data; uint32_t size; } g_amd_opticalflow%1%2%3permutations[2] = {>>%file%
for %%c in (0,1) do (
  echo     {>>%file%
  echo         g_amd_opticalflow%1%2%3permutations_%%c, sizeof^(g_amd_opticalflow%1%2%3permutations_%%c^)>>%file%
  echo     },>>%file%
)
echo };>>%file%
goto :eof
